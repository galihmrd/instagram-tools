...
class InstagramAPI:
    ...
    USER_AGENT = "Instagram 9.2.0 Android ({android_version}/{android_release}; 640dpi; 1080x2400; {manufacturer}; {model}; armani; qcom; en_US)".format(
        **DEVICE_SETTINTS
    )
    ...
    def login(self, force=False):
        if not self.isLoggedIn or force:
            ...
            if self.SendRequest(
                ...
                if self.SendRequest(
                    "accounts/login/", self.generateSignature(json.dumps(data)), True
                ):
    ...
    def follow(self, userId):
        ...
        return self.SendRequest(
            "friendships/create/" + str(userId) + "/", self.generateSignature(data)
        )
    ...
    def unfollow(self, userId):
        ...
        return self.SendRequest(
            "friendships/destroy/" + str(userId) + "/", self.generateSignature(data)
        )
    ...
    def getPopularFeed(self):
        popularFeed = self.SendRequest(
            "feed/popular/?people_teaser_supported=1&rank_token="
            + str(self.rank_token)
            + "&ranked_content=true&"
        )
        return popularFeed
